...
__device__ __inline__ NT shflType(NT val, uint32_t idx,
...
template < class NT >
__device__ __inline__ NT shflUpPred(NT val, uint32_t ofs, int32_t& pred,
...
template < class NT >
__device__ __inline__ NT shflDownPred(NT val, uint32_t ofs, int32_t& pred,
                                   uint32_t allmsk = 0xffffffffu, uint32_t shfl_c = 31)
{
    constexpr uint32_t SZ = (sizeof(NT) + sizeof(uint32_t) - 1) / sizeof(uint32_t);
    union {
