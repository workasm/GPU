...
kernel void vectorAdd(global const float *vecA, global const float *vecB, global float *vecC, uint vsize) {
...
bool GPU_playground::launch_reduce(size_t dataSz)
{
...
__device__ DeviceMatrix get_sub(DeviceMatrix A, int col, int row)
{
    DeviceMatrix Asub;
    Asub.w = min(TILE_SZ, A.w - col*TILE_SZ);
    Asub.h = min(TILE_SZ, A.h - row*TILE_SZ);
    Asub.stride = A.stride;
    Asub.data = &A.data[TILE_SZ*(A.stride * row + col)];
    return Asub;
}

// constant grid of size (BLOCK_X x BLOCK_X)
template < uint32_t TILE_SZ >
...
__global__ void mat_mul_kernel(const DeviceMatrix A,
    ...
    const uint32_t X = bidx_x * TILE_SZ + thX,
                   Y = bidx_y * TILE_SZ + thY,
                   numAw = (A.w + TILE_SZ-1) / TILE_SZ;

    auto shA = (Real *)shared, shB = shA + bx*by;

    // one thread block processes one horizontal stripe of A and one vertical stripe of B
    Real sum = 0;
    for(uint32_t i = 0, wi = 0; i < numAw; i++, wi += TILE_SZ) { // A.w = B.h: common dimension

        auto subA = get_sub<TILE_SZ>(A, i, bidx_y),
             subB = get_sub<TILE_SZ>(B, bidx_x, i);

        //Asub.data = &A.data[TILE_SZ*(A.stride * row + col)];
        //A.stride * bidx_y * TILE_SZ + i*TILE_SZ

        bool Aok = wi + thX < A.w && Y < A.h,
             Bok = X < B.w && wi + thY < B.h;
        ...
        uint32_t num = min(TILE_SZ, A.w - wi);
...
bool GPU_playground::launch_matmul()
{
